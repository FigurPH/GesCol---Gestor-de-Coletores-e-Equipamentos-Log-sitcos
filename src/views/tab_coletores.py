...
class TabColetores(wx.Panel):
    def __init__(self, parent):
        ...
        self.list_ctrl = wx.ListCtrl(   # Cria um elemento de lista controlado pelo wx
            self,
        ...
        self.list_ctrl.Bind(wx.EVT_LIST_ITEM_ACTIVATED, self.on_editar_coletor) # Duplo clique
    ...
    def carregar_coletores(self):
        ...
        for index, coletor in enumerate(self.coletores_lista):
            list_index = self.list_ctrl.InsertItem(index, str(coletor['id']).zfill(3))
            self.list_ctrl.SetItem(list_index, 1, coletor['modelo'])
            self.list_ctrl.SetItem(list_index, 2, "Sim" if coletor['disponibilidade'] else "Não")
    ...
    def on_adicionar_coletor(self, event=None):
        ...
        if dlg.ShowModal() == wx.ID_OK:
            ...
            coletor = self.controller.adicionar_coletor(id=numero, modelo=modelo, disponibilidade=disponivel)

            if coletor:
                wx.MessageBox(f"Coletor {numero} adicionado com sucesso!", "Sucesso", wx.OK | wx.ICON_INFORMATION)
                self.carregar_coletores()
        dlg.Destroy()
        self.atualizar_estado_botoes()
        return True
        ...
        if select_index != -1:
            ...
            if coletor:
                ...
                if dlg.ShowModal() == wx.ID_OK:
                    ...
                    if self.controller.editar_coletor(numero, modelo, disponivel):
                        wx.MessageBox(f"Coletor {numero} atualizado com sucesso!", "Sucesso", wx.OK | wx.ICON_INFORMATION)
                        self.carregar_coletores()
                    else: wx.MessageBox(f"Erro ao atualizar o coletor {str(numero).zfill(3)}.", "Erro",
                                        wx.OK | wx.ICON_ERROR)
                dlg.Destroy()
        self.atualizar_estado_botoes()
    ...
    def on_excluir_coletor(self, event):
        ...
        if selected_index != -1:
            ...
            if coletor:
                dlg = wx.MessageDialog(self, f"Deseja mesmo excluir o coletor {coletor_numero}?",
                                        "Confirmar Exclusão", wx.YES_NO | wx.ICON_QUESTION)
                if dlg.ShowModal() == wx.ID_YES:
                    if self.controller.excluir_coletor(coletor_numero):
                        wx.MessageBox(f"Coletor {coletor_numero} excluído com sucesso!", "Sucesso", wx.OK | wx.ICON_INFORMATION)
                        self.carregar_coletores()
                    else: wx.MessageBox(f"Erro ao excluir o coletor {coletor_numero}", "Erro", wx.OK | wx.ICON_ERROR)
            dlg.Destroy()
        self.atualizar_estado_botoes()
...
class AdiconarColetorDialog(wx.Dialog):
    def __init__(self, parent):
        ...
        sizer_campos.AddMany([
            (wx.StaticText(self, label="Número:"), 0, wx.ALIGN_RIGHT | wx.ALIGN_CENTER_VERTICAL),
            (self.txt_numero, 0, wx.EXPAND),
            (wx.StaticText(self, label="Modelo:"), 0, wx.ALIGN_RIGHT | wx.ALIGN_CENTER_VERTICAL),
            (self.txt_modelo, 0, wx.EXPAND),
            (wx.StaticText(self, label="Disponível:"), 0, wx.ALIGN_RIGHT | wx.ALIGN_CENTER_VERTICAL),
...
class EditarColetorDialog(wx.Dialog):
    ...
    def __init__(self, parent, coletor):
        ...
        # --- Elementos da Interface ---
        self.txt_numero = wx.TextCtrl(self, value=str(self.coletor.id), style=wx.TE_READONLY) # Não permitir editar o ID
