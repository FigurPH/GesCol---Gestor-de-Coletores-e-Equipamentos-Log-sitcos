...
class TabColaboradores(wx.Panel):
    ...
    def on_adicionar_colaborador(self, event):
        ...
        if dlg.ShowModal() == wx.ID_OK:
            ...
            colaborador = self.controller.adicionar_colaborador(matricula, nome, cargo, autorizado_transp, autorizado_emp)
            if colaborador:
                wx.MessageBox(f"Colaborador {nome} adicionado com sucesso!", "Sucesso", wx.OK | wx.ICON_INFORMATION)
                self.carregar_colaboradores()
        dlg.Destroy()
        self.atualizar_estado_botoes()
    ...
    def on_editar_colaborador(self, event):
        ...
        if selected_index != -1:
            ...
            print(f"Tentando editar colaborador com matrícula: {matricula}") # DEBUG
    ...
    def on_excluir_colaborador(self, event):
        ...
        if selected_index != -1:
            matricula = self.list_ctrl.GetItemText(selected_index, 0)
            print(f"Tentando excluir colaborador com matrícula: {matricula}") # DEBUG
            colaborador = self.controller.buscar_colaborador(matricula)
            if colaborador:
